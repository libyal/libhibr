...
LIBRARY_TESTS="compressed_page_data compressed_page_map error io_handle notify";
...
run_test()
{
	...
	local TEST_EXECUTABLE="./hibr_test_${TEST_NAME}";
...
run_test_with_input()
{
	...
	run_test_on_input_directory "libhibr" "${TEST_DESCRIPTION}" "default" "${OPTION_SETS}" "${TEST_EXECUTABLE}" "input" "${INPUT_GLOB}";
	local RESULT=$?;
