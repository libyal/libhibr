 ...
 */
int hibr_test_single_open_close_file(
     ...
     int expected_result )
{
	...
	static char *function    = "hibr_test_single_open_close_file";
	char *access_string      = NULL;
	int result               = 0;

	if( access_flags == LIBHIBR_OPEN_READ )
	{
		access_string = "read";
	}
	else if( access_flags == LIBHIBR_OPEN_WRITE )
	{
		access_string = "write";
	}
	else
 ...
 * Returns 1 if successful, 0 if not or -1 on error
 */
int hibr_test_multi_open_close_file(
     libcstring_system_character_t *filename,
     int access_flags,
     int expected_result )
{
	libcerror_error_t *error = NULL;
	libhibr_file_t *file     = NULL;
	static char *function    = "hibr_test_multi_open_close_file";
	char *access_string      = NULL;
	int result               = 0;
...
#endif
{
...
#if defined( HAVE_DEBUG_OUTPUT ) && defined( HIBR_TEST_OPEN_CLOSE_VERBOSE )
	 ...
	 */
	if( hibr_test_single_open_close_file(
	     source,
	     LIBHIBR_OPEN_READ,
	     1 ) != 1 )
	{
	 ...
	 */
	if( hibr_test_multi_open_close_file(
	     source,
	     LIBHIBR_OPEN_READ,
	     1 ) != 1 )
	{
