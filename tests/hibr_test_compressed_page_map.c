/*
 * Library compressed_page_map type test program
 *
...
#include "../libhibr/libhibr_compressed_page_map.h"

#if defined( __GNUC__ ) && !defined( LIBHIBR_DLL_IMPORT )

/* Tests the libhibr_compressed_page_map_initialize function
 * Returns 1 if successful or 0 if not
 */
int hibr_test_compressed_page_map_initialize(
     void )
{
	...
	/* Test regular cases
	 */
	result = libhibr_compressed_page_map_initialize(
	          &compressed_page_map,
	          &error );
	...
	result = libhibr_compressed_page_map_free(
	          &compressed_page_map,
	          &error );
	...
	compressed_page_map = (libhibr_compressed_page_map_t *) 0x12345678UL;
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libhibr_compressed_page_map_initialize with malloc failing
		 */
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libhibr_compressed_page_map_initialize with memset failing
		 */
...
/* Tests the libhibr_compressed_page_map_free function
 * Returns 1 if successful or 0 if not
 */
int hibr_test_compressed_page_map_free(
     void )
{
	libcerror_error_t *error = NULL;
	int result               = 0;
...
#endif
{
	...
	HIBR_TEST_RUN(
	 "libhibr_compressed_page_map_initialize",
	 hibr_test_compressed_page_map_initialize );

	HIBR_TEST_RUN(
	 "libhibr_compressed_page_map_free",
	 hibr_test_compressed_page_map_free );

	/* TODO: add tests for libhibr_compressed_page_map_read */
