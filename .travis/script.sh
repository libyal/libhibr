...
then
	docker run -t -v "${PWD}:/libhibr" ${DOCKERHUB_REPO}:${DOCKERHUB_TAG} sh -c "cd libhibr && .travis/script_docker.sh";
...
then
	...
	then
		...
		cp AUTHORS COPYING NEWS README ${PWD}/osx-pkg/usr/share/doc/libhibr;
