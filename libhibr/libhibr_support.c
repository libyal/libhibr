...
const char *libhibr_get_version(
             void )
{
	return( (const char *) LIBHIBR_VERSION_STRING );
}
...
int libhibr_get_access_flags_read(
     void )
{
	return( (int) LIBHIBR_ACCESS_FLAG_READ );
}
...
int libhibr_get_access_flags_read_write(
     void )
{
	return( (int) ( LIBHIBR_ACCESS_FLAG_READ | LIBHIBR_ACCESS_FLAG_WRITE ) );
}
...
int libhibr_get_access_flags_write(
     void )
{
	return( (int) LIBHIBR_ACCESS_FLAG_WRITE );
}
...
{
	static char *function = "libhibr_get_codepage";
...
int libhibr_set_codepage(
     ...
     libcerror_error_t **error )
{
	static char *function = "libhibr_set_codepage";
...
/* Determines if a file is a HIBR file (check for the HIBR file signature)
     ...
     libcerror_error_t **error )
{
	...
	static char *function            = "libhibr_check_file_signature";
...
{
	libbfio_handle_t *file_io_handle = NULL;
	static char *function            = "libhibr_check_file_signature_wide";
	size_t filename_length           = 0;
	int result                       = 0;
...
/* Determines if a file is a HIBR file (check for the HIBR file signature) using a Basic File IO (bfio) handle
 * Returns 1 if true, 0 if not or -1 on error
 */
     ...
     libcerror_error_t **error )
{
	...
	static char *function      = "libhibr_check_file_signature_file_io_handle";
...
/* TODO check for the other signatures */
	if( memory_compare(
	     "hibr",
	     signature,
	     4 ) == 0 )
